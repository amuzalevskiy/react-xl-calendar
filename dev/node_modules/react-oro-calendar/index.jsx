...
var ReactOroCalendar = {
    CalendarDataModel: function() {
        return {
            calendars: [],
            events: []
        }
    },
    CalendarModel: function(name){
        return {
            name: name
        }
    },
    EventModel: function(name, start, end,calendar){
        return {
            start: start,
            end: end,
            calendar: calendar,
            ...
            return <div className="horizontal-events-line-wrapper">
                {layout.map((line)=> (
                    <div className="horizontal-events-line">{line.map((eventData)=> {
                        var relativeStart = Math.floor((eventData.e.start - this.state.startOfWeek) / (24*60*60*1000));
                        var relativeEnd = Math.ceil((eventData.e.end - this.state.startOfWeek) / (24*60*60*1000));
            ...
            if (dragContext.data) {
                ...
                if (snapDelta !== 0 || dragContext.projection) {
                    ...
                    start.setUTCFullYear(
                        this.state.currentDay.getUTCFullYear(),
                        this.state.currentDay.getUTCMonth(),
                        this.state.currentDay.getUTCDate()
                    );
